                ...
                // xdmp.log("======="+comment.xpath("*:new-to-be-fixed-in-version/text()").toString())
                 ...
                 if(comment.xpath("*:assign-by/text()").toString()){  
                       change.change.assignTo = {
                        from: getUserInfo(users,comment.xpath("*:assign-by/text()")) || '' ,    
                         to: getUserInfo(users, comment.xpath("*:assigned-to/text()")) 
                            }
                          change.show = true;
                       }
       ...
       // no stacktrace  
        ...
        newbug.note = bug.environment.note || ''  
         newbug.subscribers = [newbug.submittedBy] 
            ...
            newbug.parent = {
              type: bug.relationships.relation.type,
              id: parseInt(bug.relationships.relation.to),
            }   
            newbug.includeInTaskList = JSON.parse(bug['task-list'].include) || false;
...
var users = fn.doc("root/support/bugtracking/users.xml").next().value.root
var uris = cts.uriMatch("root/support/bugtracking/bug32251.xml")
 var convertedDoc;   
     for (uri of uris){
      ...
      convertedDoc = convertBug(doc)
       loadConvertedDoc(convertedDoc)
       // if the bug is a clone, then update parent bug
       if(convertedDoc.cloneOf){
         var parent = xdmp.eval("print(uri); cts.doc(uri)", {uri: '/bug/'+convertedDoc.cloneOf+ '/'+ convertedDoc.cloneOf+ '.json'}, {database: xdmp.database('bugtrack')}).next().value.root.toObject()
       // if clones array exist then push else create it  
       if(parent.clones){
             parent.clones.push(convertedDoc.id);       
         } else {
           parent.clones = [convertedDoc.id]
         }  
          loadConvertedDoc(parent)
       }
       }
       if(kind.toString() === 'Task'){
           loadConvertedDoc(convertTask(doc))
        }
    
    if(kind.toString() === 'RFE'){
           loadConvertedDoc(convertTask(doc))
        }
